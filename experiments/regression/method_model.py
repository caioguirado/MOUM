...
from ...models.regression.algorithm_adaptation import ReducedRankRegression
...
class MethodModel(Experiment):
    ...
    def fit_method_obj(self, dataset, method, model, method_obj):
            cv_results = []
    ...
    def run(self):
        ...
        for method in tqdm(dimensions['methods']):
            for model in dimensions['models']:
                ...
                result = self.fit_method_obj(dataset=dataset, method=method, model=model, method_obj=method_obj)
                results.append(result)

        result = self.fit_method_obj(dataset=dataset, 
                                        method='RRR', 
                                        model={'enum': 'RRR'}, 
                                        method_obj=ReducedRankRegression())
        results.append(result)
